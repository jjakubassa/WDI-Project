 ...
 * 
 */
public class LabelsFuserVoting extends AttributeValueFuser<List<String>, Album, Attribute> {

	public LabelsFuserVoting() {
		super(new Voting<List<String>, Album, Attribute>());
	}
	
	@Override
	public boolean hasValue(Album record, Correspondence<Attribute, Matchable> correspondence) {
		return record.hasValue(Album.LABELS);
	}
	
	@Override
	public List<String> getValue(Album record, Correspondence<Attribute, Matchable> correspondence) {
		return Arrays.asList(record.getLabels());
	}
	...
	public void fuse(RecordGroup<Album, Attribute> group, Album fusedRecord, Processable<Correspondence<Attribute, Matchable>> schemaCorrespondences, Attribute schemaElement) {
	    ...
	    Set<String> uniqueLabels = new LinkedHashSet<>(fused.getValue());
	    
	    String[] uniqueLabelsArray = uniqueLabels.toArray(new String[0]);

	    fusedRecord.setLabels(uniqueLabelsArray);
	    fusedRecord.setAttributeProvenance(Album.LABELS, fused.getOriginalIds());
	}
